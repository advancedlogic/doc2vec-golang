...
import (
	...
	"github.com/advancedlogic/doc2vec-golang/corpus"
	"github.com/advancedlogic/doc2vec-golang/neuralnet"
...
func (p *TDoc2VecImpl) TrainFromFolder(fname string) {
	p.Trainfile = fname
	p.Corpus.BuildFromFolder(fname)
	if p.UseNEG {
		p.initUnigramTable()
	}
	p.NN = neuralnet.NewNN(p.Corpus.GetDocCnt(), p.Corpus.GetVocabCnt(), p.Dim, p.UseHS, p.UseNEG)
